...
# 读取CSV文件
train_csv_path = file_dir(__file__,"mouse_data.csv")  # 替换为你的CSV文件路径
train_csv = pd.read_csv(train_csv_path, header=None)
test_csv_path = file_dir(__file__,"mouse_data_test.csv")  # 替换为你的CSV文件路径
test_csv = pd.read_csv(test_csv_path, header=None)

# 提取dx, dy和标签
dx_dy_train = train_csv.iloc[:, 0].apply(lambda x: list(map(int, x.split(","))))
dx_dy_labels_train = train_csv.apply(
    lambda row: [list(map(int, row[i].split(","))) for i in range(1, 11)], axis=1
)
dx_dy_test = test_csv.iloc[:, 0].apply(lambda x: list(map(int, x.split(","))))
dx_dy_labels_test = test_csv.apply(
    lambda row: [list(map(int, row[i].split(","))) for i in range(1, 11)], axis=1
)
